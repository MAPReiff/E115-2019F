...
int main()
{
	...
	if (opt == 1) {
		...
		if (finNum > 100) {
			...
			finAv = finAv / finNum;
			finF = finAv * (finWorth / 100);

		}

		float finalGrade = finF + midF + hwF;

		if (finalGrade > 100) { //over 100 => extra credit
			cout << "Your final grade is " << finalGrade << "! Thats an A+++++!";
		}
		else if (finalGrade >= 91 && finalGrade <= 100) { //score range of 91-100 is an A
			cout << "Your final grade is " << finalGrade << "! Thats an A!";
		}
		else if (finalGrade >= 81 && finalGrade <= 90) { //score range of 81-90 is a B
			cout << "Your final grade is " << finalGrade << "! Thats a B!";
		}
		else if (finalGrade >= 71 && finalGrade <= 80) {  //score range of 71-80 is a C
			cout << "Your final grade is " << finalGrade << "! Thats a C!";
		}
		else if (finalGrade >= 65 && finalGrade <= 70) { //score range of 65-71 is a D
			cout << "Your final grade is " << finalGrade << "! Thats a D!";
		}
		else if (finalGrade < 65) { //score of under 65 is an F and they must repeat the class.
			cout << "Your final grade is " << finalGrade << "! See you again next semester ;)";
		}
